...
Run **Lean Comparator, Candidate Fate, and Cooperative Queue Development v1** on the existing frozen corpus and four held-out claims. Runtime revision 6 cannot truthfully migrate the v7 Bundle because v7 never retained Candidate Fates or queue attribution; create one bounded replacement v10 training Bundle under the unchanged corpus, model, features, and Verification envelope. This is a required semantic migration, not a trainer or capacity treatment. It starts with a no-kernel audit and then uses two short search phases.

Implementation status: the shared Operational Policy module and frozen mixed-prefix tests establish that operational Bootstrap and a zero FTRL predictor select the same Candidate order when forecasts contain no ranking information, while deliberately recording their first policy-identity divergence: the first unprotected Candidate is attributed to Learned by the 1:1 learned-first cooperative rule and to Bootstrap by the actual Bootstrap branch. The [v11 result](../experiments/lean-public-optimizer-development-v11.md) recovered all four known heldout collapses in every treatment, proving that the v7 loss occurred before selection. It also found no causal discovery benefit from the active model, while Full used more CPU than no-model and Bootstrap. Every success came from protected per-claim selection, and one-batch Verification made every registered cutoff share the same batch CPU. Runtime revision 10 closes that feedback delay with bounded [Verification Cohorts](../adr/0064-close-the-search-feedback-loop-with-verification-cohorts.md): exact deferred work and newly eligible parents are restart-complete before allocation is reconsidered, and crash tests establish equivalence with uninterrupted execution. The Lean domain now indexes verified donors by exact proposition and symbol-independent structural relevance before bounded enumeration, preserves donor provenance and relation features in Experience, and charges index and bounded scratch memory to the Resource Envelope. The [v1 retrieval diagnostic](../experiments/lean-premise-retrieval-development-v1.md) placed all four known shorter proofs at rank one and measured a greater than 3,000-fold synthetic query speedup over rescanning. This advances Lean's Semantic Identity and still requires an equal-budget public-path causal test. Fixed-Experience promotion remains explicitly diagnostic; only the equal-budget public-path treatment is causal.

The equal-budget [v18 public-path result](../experiments/lean-public-optimizer-development-v18.md) now establishes causal value for the active donor-aware linear model on the four-claim Development opportunity corpus. Full found all four strict proof collapses within 16 Candidate Verification requests; Bootstrap found one at that prefix and three after 124 observed Candidate requests; no-model found one at the prefix and two overall. At request 16, Full used 0.518 seconds of Verification-batch CPU per discovery versus Bootstrap's 1.349 seconds. No-derived matched Full, localizing the gain to Experience-trained ranking rather than chained derived artifacts. This clears the model-activation gate but not the model-capacity gate: the next test broadens independent correctness claims under fixed total Verification, then compares claim-conditioned ranking objectives on identical Experience. Report schema v19 also corrects the primary selector's accidental admission of generated `_auxLemma.N` training declarations and separates exact request-prefix efficiency from setup-dominated end-to-end CPU.

The corrected [v19 reproduction](../experiments/lean-public-optimizer-development-v19.md) replaces both generated training declarations, promotes a different generation-one model from 714 labeled attempts, and reproduces the same 4/3/2 Full/Bootstrap/no-model outcome. Full again finds all four by request 16, at 0.514 seconds of Verification-batch CPU per discovery versus Bootstrap's 1.309 seconds. This closes the corpus-eligibility concern while retaining the same four-claim Development limit.
