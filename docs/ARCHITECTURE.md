...
On a multi-claim Search Frontier, Candidate allocation first reserves one exploration slot for every represented Seed-relative correctness claim when the remaining Verification budget can afford the complete set. A protected Derived Operator Candidate satisfies that obligation preferentially because it can combine coverage with a verified discovery; learned ranking, uncertainty exploration, and remaining Derived Operator protection allocate only the remaining slots. Thus a large or initially attractive Seed cannot erase every observation from another claim, while single-claim frontiers pay no diversity tax. When the budget cannot cover every claim, active Preference and ordinary ranking retain authority instead of an arbitrary partial reservation overriding caller economics.
