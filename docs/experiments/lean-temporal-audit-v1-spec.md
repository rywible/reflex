...
The audit snapshot is the latest mathlib commit whose commit timestamp is strictly before 2026-07-01T00:00:00Z. At lock creation, the harness queries GitHub’s primary commits API for exactly the newest default-branch commit through 2026-06-30T23:59:59Z, verifies that the returned SHA is the requested pin, and records the response URL, timestamp, and SHA-256. Its selected Lean toolchain, local toolchain alias, runtime version, and Lean commit are also recorded in the content-addressed lock before checkout. This is the only networked preparation step; confirmation is fully local. The final command builds the audit catalog and analyzes it in one execution. A repository-fixed receipt consumes the seal, so changing output paths cannot create another attempt. Failure, timeout, or any deviation is retained in a fixed failure report and cannot be discarded or rerun in pursuit of a favorable result.
