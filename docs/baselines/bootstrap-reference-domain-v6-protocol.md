# Bootstrap Reference Domain Baseline v6 Protocol

Status: frozen exploratory protocol

Protocol version: `reflex-bootstrap-baseline-v6`

Protocol SHA-256 for the declared 1/2/4/8-worker matrix: `64c4b2a3eab20845cdba4ce267f673330a12d656f4bb9595200ad869bde4e2a1`
...
This protocol supersedes v5 after recovery validation was corrected to treat immutable predecessor Knowledge Revision counters as historical prefixes rather than requiring them to equal the current Experience Ledger. The search, Verification, Measurement, and baseline corpus semantics are unchanged. V5 remains immutable evidence for the pre-fix revision.

Version 6 freezes the whole-Session public-`improve` workload against Semantic Identity `reflex-bitvec/u8/unary/full-ops/masked-shifts/select-nonzero/canonical-dag/v3`. It is an Exploratory Run under `docs/EXPERIMENTS.md`, not Scientific Confirmation. Every assignment starts with empty Knowledge and the Bootstrap Model; learning and consolidation occur only after that assignment's search and are tested through completed recovery.
