# Bootstrap Reference Domain Baseline Protocol

Status: frozen exploratory protocol

Protocol version: `reflex-bootstrap-baseline-v1`

Protocol SHA-256 for the declared 1/2/4/8-worker matrix: `344cd94cae2acd93e39d7a5ce0691cad73786265b84b0fb9a5dab1b34d2e6541`
...
This protocol freezes the whole-Session Bootstrap comparator before Reflex adds a learned challenger. It is an Exploratory Run under `docs/EXPERIMENTS.md`; it does not provide Scientific Confirmation and cannot be used as a confirmatory audit corpus.
...
The corpus contains all 256 unary `u8` semantic functions `x xor c`, one for each `c` in `0..=255`. Each function is presented through the same two-layer reducible `xor(_, 0)` lineage. The complete function family, not surface rows, is the unit of corpus construction. The harness hashes the actual canonical Artifact encodings in constant order.
...
Every assignment starts a fresh process and invokes the public `improve` operation. It then invokes a completed `Resume` against the same bundle to measure import, Verification replay, transient-index reconstruction, and atomic republication. Warmups are marked and retained in raw output but excluded from summaries.
...
The checked-in report is the immutable Bootstrap comparator for the first FTRL operational gate. Later performance investigations may create new versioned protocols; they do not overwrite this one.
