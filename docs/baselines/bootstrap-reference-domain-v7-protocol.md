# Bootstrap Reference Domain Baseline v7 Protocol

Status: frozen exploratory protocol

Protocol version: `reflex-bootstrap-baseline-v7`

Protocol SHA-256 for the declared 1/2/4/8-worker matrix: `14b108b35f336c134fa94139de5430adba0904f8471ffa44de4fb7c55fc0b880`
...
This protocol supersedes v6 after Knowledge Revisions gained an explicit Experience-ledger watermark. Recovery now validates each immutable revision exactly against the append-only prefix it claims to summarize, including campaign-scale Sessions where the active-knowledge cap legitimately prevents a new promotion. The search, Verification, Measurement, and baseline corpus semantics are unchanged. V6 remains immutable evidence for the pre-format revision.

Version 7 freezes the whole-Session public-`improve` workload against Semantic Identity `reflex-bitvec/u8/unary/full-ops/masked-shifts/select-nonzero/canonical-dag/v3`. It is an Exploratory Run under `docs/EXPERIMENTS.md`, not Scientific Confirmation. Every assignment starts with empty Knowledge and the Bootstrap Model; learning and consolidation occur only after that assignment's search and are tested through completed recovery.
