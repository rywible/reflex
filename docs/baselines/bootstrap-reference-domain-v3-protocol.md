# Bootstrap Reference Domain Baseline v3 Protocol

Status: frozen exploratory protocol

Protocol version: `reflex-bootstrap-baseline-v3`

Protocol SHA-256 for the declared 1/2/4/8-worker matrix: `288979cf8ace298879d166859a1d40a7ab43341d49fe848002c64151bdfe3f82`
...
This protocol supersedes the v2 Bootstrap performance comparator after the production Runtime gained Knowledge Consolidation, bounded live Artifact indexes, and verified Derived Operators. The v2 protocol and report remain immutable historical evidence for the pre-consolidation Runtime.

Version 3 freezes the same whole-Session, public-`improve` workload against the complete pre-causal-gate Runtime. It is an Exploratory Run under `docs/EXPERIMENTS.md`; it is not Scientific Confirmation and cannot serve as a sealed audit corpus. Each fresh Campaign remains pinned to empty Knowledge and Bootstrap Model Revisions throughout measured search. Consolidation and training occur only after search and can affect the completed bundle and its recovery, never the measured fresh Campaign's allocation.
...
Every assignment starts a fresh process and invokes the public `improve` operation. It then invokes a completed `Resume` against the same bundle to measure Artifact, Seed, and Experience Verification replay, transient-index reconstruction, Knowledge/Model validation, and atomic republication. Warmups are marked and retained in raw output but excluded from summaries.
...
The resulting versioned report is the Bootstrap comparator for the complete production Runtime before the preregistered causal gate. Later investigations create new versioned protocols and never overwrite this record.
